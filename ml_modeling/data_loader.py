...
def load_data(config_path=config_path):
    ...
    # Keeping intersection of indices; order clinical by radiomics_delta order
    common_idx = radiomics_delta.index.intersection(clinical.index)
    clinical = clinical.reindex(common_idx)
    # Creating a dataframe for the target variables and dropping them from the clinical dataframe to avoid data leakagg
    ...
    clinical = clinical.drop(columns=targets_to_drop)
